    ...
    def __init__(self,
        ...
        self.roi_extractor = SingleRoIExtractor(
            roi_layer={
                ...
                'aligned': False,    # GlobalTrack version use False
...
class QG_RCNN(TwoStageDetector):    # pylint: disable=invalid-name
    ...
    def simple_test_bboxes(self,
        ...
        for proposals in proposals_list:
            ...
            # print(bbox_feats_x[0, 0, :3, :3], bbox_feats_x.shape)
            # fsd
            ...
            det_bboxes, _ = self.roi_head.bbox_head.get_bboxes(
                rois_x,
                cls_score,
                bbox_pred,
                img_shape,
                ...
                rescale=True,
                cfg=rcnn_test_cfg)
            rough_det = det_bboxes.cpu().numpy()
...
class GlobalTracker(Tracker, BaseLocalizer):    # pylint: disable=abstract-method
    ...
    def __init__(self, gpus, _cfg):
        ...
        global_tracker_cfg.model.test_cfg.rcnn.max_per_img = _cfg.INFERENCE.LOCALIZER_TOPK
        global_tracker_cfg.model.test_cfg.rcnn.nms.iou_thr = _cfg.EVAL.NMS
        self.model = self._build_from_cfg(global_tracker_cfg.model,
                                          test_cfg=global_tracker_cfg.model.test_cfg)
    ...
    def set_query(self, query_img_bgr, query_bbox, query_instance_id):
        ...
        self.bbox_feats_z_dic[query_instance_id] = bbox_feats_z
    ...
    def process_gallery_memory(self, img_x, img_meta_x):
        ...
        proposal_list = self.model.rpn_head.simple_test_rpn(
    ...
    def process_gallery_single_query(self, img_x, img_meta_x):
        ...
        for query_instance_id in self.activated_query_list:
            ...
            # proposal_list = self.model.rpn_head.simple_test_rpn(
            #     rpn_feats, img_meta_x, self.model.test_cfg.rpn)
            # merged_meta_x = [{**img_meta_x[0], **self.model.test_cfg.rpn}]
            proposal_list = self.model.rpn_head.simple_test_rpn(
                rpn_feats, img_meta_x)
