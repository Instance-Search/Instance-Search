...
data = dict(
    ...
    val=dict(
        type='PairWrapper',
        ann_file=None,  # for compatibility
        base_dataset='Cityflow_test',
        base_transforms='basic_test',
        sampling_prob=[1.0],
        max_size=10000,
        max_instances=8,
        with_label=True)
...
lr_config = dict(
    ...
    warmup_ratio=1. / 3,
    step=[2, 3])
checkpoint_config = dict(interval=1)    # origin 1
log_config = dict(
    interval=50,
...
load_from = 'log/baseline/qg_rcnn_r50_fpn_2x_20181010-443129e1.pth'
resume_from = None
workflow = [('train', 1)]
