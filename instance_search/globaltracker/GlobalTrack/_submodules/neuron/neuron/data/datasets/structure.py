...
class Seq2Pair(PairDataset):
    ...
    def __init__(self, seqs, transforms=None,
        ...
        self.fixed_train_pair_num = 2000
        # TODO, set by outside
        self.sample_strategy = 'topk'
        if 'train' in seqs.name:
            self.group_flags = np.ones(self.fixed_train_pair_num, dtype=np.uint8)
            # pairs_per_seq = 1
            # self.pairs_per_seq = pairs_per_seq
            # self.group_flags = self._build_group_flags(seqs, pairs_per_seq)
        else:
            pairs_per_seq = max(int(10000 / len(self.seqs)), 1)
            # pairs_per_seq = 10
            self.pairs_per_seq = pairs_per_seq
            self.fixed_test_pair = self._get_fixed_test_pair()
            self.group_flags = np.ones(len(self.fixed_test_pair), dtype=np.uint8)
        self.max_distance = max_distance
        # group sequences by aspect ratios (for sampling)

    def _get_fixed_test_pair(self):
        """Get fixed test pair for re-produceable results."""

        fixed_test_pair = []
        short_fixed_test_pair = []
        for seq_data in self.seqs:
            img_files, target = seq_data
            anno, meta = target['anno'], target['meta']
            # filter noisy annotations
            valid_indices = self._filter(anno, meta)
            # valid_indices = np.arange(len(anno))    # no filter for test
            if len(valid_indices) < 2:
                # this filters a lot of test pair for CUHK-SYSU
                # print('invalid test data with only one valid instance!')
                continue
            # first one is query and the other are gallery
            name_z = img_files[valid_indices[0]]
            anno_z = anno[valid_indices[0]]
            for index, valid_indice in enumerate(valid_indices[1:]):
                name_x = img_files[valid_indice]
                anno_x = anno[valid_indice]
                fixed_test_pair.append([name_z, anno_z,
                                        name_x, anno_x])
                if index < self.pairs_per_seq:
                    short_fixed_test_pair.append([name_z, anno_z,
                                                  name_x, anno_x])
        # TODO: deadlock if full fixed_test_pair
        # we just clip lenth as a workaround.
        if len(fixed_test_pair) > 10000:
            print(f'too many test pairs ({len(fixed_test_pair)}),'\
                  'will clip GT by each query')
            fixed_test_pair = short_fixed_test_pair
        print(f'{len(fixed_test_pair)} fixed test pairs')
        return fixed_test_pair

    def __getitem__(self, index):
        if 'train' in self.seqs.name:
    ...
    def _get_test_item(self, index):
        ...
        name_z, anno_z, name_x, anno_x = self.fixed_test_pair[index]
        bboxes_z = np.expand_dims(anno_z, axis=0)
        bboxes_x = np.expand_dims(anno_x, axis=0)

        # image pair
        img_z = ops.read_image(name_z)
        img_x = ops.read_image(name_x)
    ...
    def _get_train_item(self, index):
        if index > len(self):
            raise IndexError('Index out of range')
        # index %= len(self.seqs)
        index = random.randint(0, len(self.seqs)-1)

        # get image files and annotations
        img_files, target = self.seqs[index]
        anno, meta = target['anno'], target['meta']
        if self.sample_strategy == 'rss':
            sample_prob = target['sim_list']
        else:
            sample_prob = np.ones(len(img_files))
        # filter noisy annotations
        valid_indices = self._filter(anno, meta)
        # valid_indices = np.arange(len(anno))    # no filter for train
        if len(valid_indices) < 2:
            return self._random_next()

        # ramdomly sample a pair
        sample_prob = sample_prob[valid_indices]
        rand_z, rand_x = self._sample_pair(anno, valid_indices, index, sample_prob)
    ...
    def __len__(self):
        if 'train' in self.seqs.name:
            ...
            return self.fixed_train_pair_num    # fixed lenth, may be underfit
        else:
            return len(self.fixed_test_pair)
    
    def _sample_pair(self, anno, indices, index, sample_prob):
        ndims = anno.shape[1]
        n = len(indices)
        ...
        if ndims in [4, 8]:
            if n == 1:
                ...
                if 'train' in self.seqs.name:
                    # norm again because of filter, sample two image randomly
                    # sample_prob = (sample_prob + 1.0) / 2.0
                    # sample_prob = sample_prob / np.sum(sample_prob)
                    # rand_z, rand_x = np.random.choice(indices, 2, p=sample_prob)

                    # also fix image_z for training
                    rand_z = indices[0]
                    sample_prob = (sample_prob + 1.0) / 2.0
                    sample_prob = sample_prob[1:]
                    sample_prob = sample_prob / np.sum(sample_prob)
                    rand_x = np.random.choice(indices[1:], 1, p=sample_prob)[0]
                else:
                    rand_z = indices[0]
                    rand_x = indices[(index+1) % n]
        elif ndims == 9:
            anno = anno[indices]
