...
def parse_args():
    ...
    parser.add_argument('--sampler', type=str, default='RandomSampler')
    parser.add_argument('--dataset_name', type=str, default='Cityflow')
    parser.add_argument('--imgs_per_gpu', type=int, default=2)
    parser.add_argument('--max_size', type=int, default=10000)
    parser.add_argument('--sample_strategy', type=str, default='rss')
...
def main():
    ...
    if args.autoscale_lr:
        # Ref: https://mmdetection.readthedocs.io/en/v1.2.0/GETTING_STARTED.html#train-a-model
        cfg.optimizer['lr'] = cfg.optimizer['lr'] * cfg.gpus / 8.

    # if args.sampler:
    #     cfg.train_cfg.rcnn.sampler.type = args.sampler
    # if args.lr:
    #     cfg.optimizer.lr = args.lr
    if args.dataset_name:
        cfg.data.train.base_dataset = f'{args.dataset_name}_train'
        cfg.data.val.base_dataset = f'{args.dataset_name}_test'
    # if args.imgs_per_gpu:
    #     cfg.data.imgs_per_gpu = args.imgs_per_gpu
    # if args.max_size:
    #     cfg.data.train.max_size = args.max_size
    if args.sample_strategy:
        cfg.data.train.sample_strategy = args.sample_strategy
