...
def save_pickle(_cfg, seq_dict, mode):
    ...
    pkl_dir = f'./instance_search/globaltracker/GlobalTrack/cache/{_cfg.SELF_TRAIN.MODE}'
...
class SelfTrainDataMaker:
    ...
    def make_one_seq_reid(self, query_index, pred_dic, query_label):
        ...
        unsup_reid_dir = f'{self._cfg.PATH.SEARCH_DIR}../'\
                         f'{self._cfg.SELF_TRAIN.MODE}/trainval'
    ...
    def make_selftrain_noisy_reid(self, query_instance_dic):
        ...
        selected_test_id = selected_test_id[:_cfg.EVAL.TEST_QUERY_NUM]
    ...
    def make_one_seq_loc(self, pred_dic, query_label):
        ...
        meta.update({
            ...
            'target_num': 1,
            'total_instances': len(bboxes)})
        if self._cfg.SELF_TRAIN.LOC_MODE == 'rss':
            sim_list = np.array(sim_list) / np.sum(sim_list)
        else:
            sim_list = np.ones(len(sim_list))
    ...
    def make_selftrain_clean(self):
        """Make self train clean data."""

        _cfg = self._cfg
        if 'reid' in _cfg.SELF_TRAIN.MODE:
            self.make_clean_reid()
        elif 'loc' in _cfg.SELF_TRAIN.MODE:
            # Note: What should we do for train/eval localizer alone?
            # self.make_clean_loc()    # no label for clean data.
            # Train and test share the same data.
            data_mode = _cfg.EVAL.DATASET_NAME
            pkl_dir = f'./instance_search/globaltracker/GlobalTrack/'\
                      f'cache/{_cfg.SELF_TRAIN.MODE}'
            train_pkl_name = os.path.join(pkl_dir, f'{data_mode}_train.pkl')
            test_pkl_name = os.path.join(pkl_dir, f'{data_mode}_test.pkl')
            shutil.copy(test_pkl_name, train_pkl_name)
        else:
            print(f'unknown {_cfg.SELF_TRAIN.MODE}')

    def make_clean_reid(self):
        """Copy cropped reid images as clean_gt."""

        _cfg = self._cfg
        large_image_dir = _cfg.PATH.SEARCH_DIR
        unsup_reid_dir = large_image_dir + f'../{_cfg.SELF_TRAIN.MODE}/trainval'
        gt_images = glob(large_image_dir + '../reid_images/test_gallery/*.jpg')+\
                    glob(large_image_dir + '../reid_images/test_probe/*.jpg')
        if not os.path.exists(unsup_reid_dir):
            os.makedirs(unsup_reid_dir)
        for item in gt_images:
            shutil.copy(item, unsup_reid_dir)

    def make_clean_loc(self):
        """Convert instance search label without ID for train."""

        large_image_dir = os.path.join(self._cfg.PATH.SEARCH_DIR,
                                       'test_gallery')
        _, gallery_dic = pickle.load(open(self._cfg.EVAL.LABEL_FILE, 'rb'))
        seq_dict = {}
        img_files = []
        bboxes = []
        meta = {}
        for object_data_list in gallery_dic.values():
            for sample_data in object_data_list:
                img_file = os.path.join(large_image_dir,
                                        sample_data['image_name'])
                img = self.all_image_dic[img_file]
                img_files.append(img_file)
                bboxes.append(sample_data['bbox'].astype('float32'))
...
def main():
    ...
    noisy_flag = 'noisy' in cfg.SELF_TRAIN.MODE
    sup_flag = 'sup' in cfg.SELF_TRAIN.MODE
    loc_flag = 'loc' in cfg.SELF_TRAIN.MODE
    reid_flag = 'reid' in cfg.SELF_TRAIN.MODE
    if loc_flag:
        label_file = cfg.EVAL.LABEL_FILE
        query_instance_dic = pickle.load(open(label_file, 'rb'))
        selftrain_maker.make_test_loc(query_instance_dic)
